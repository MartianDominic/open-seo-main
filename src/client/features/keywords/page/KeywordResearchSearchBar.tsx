        ...
        >
          <controlsForm.Field name="keyword">
            {(field) => {
              ...
              return (
                <label
                  className={`flex items-center gap-2 flex-1 min-w-0 max-w-md rounded-md border bg-background px-3 h-9 ${keywordError ? "border-destructive" : "border-input"}`}
                >
                  <Search className="size-4 shrink-0 text-muted-foreground" />
                  <Input
                    className="border-0 p-0 h-auto shadow-none focus-visible:ring-0 grow min-w-0 bg-transparent"
          ...
          <controlsForm.Field name="locationCode">
            {(field) => (
              <select
                className="h-8 rounded-md border border-input bg-background px-2.5 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-ring w-auto"
                value={field.state.value}
                onChange={(event) =>
